# /reader-qa — Reader-Proxy QA runbook

Usage: `/reader-qa <slug> [--gripe-pass] [--force]`. The default reader-facing QA, replacing
`/review-node` score panels. `/reader-proxy` is an alias for the same runbook.

Canonical methodology: `docs/READER-QA.md`. This is the reader-facing half of QA; the Logic
Sweep (`/logic-sweep`) is the continuity half. **Emits no scores** — measurements and
page-anchored findings only.
...
- Never run `--review-node` / persona panels / votes as part of this flow —
