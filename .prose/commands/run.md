...
Launches the Prose.Writer app and opens it in Chrome at the local dev URL. No arguments needed.

Fixed facts:
- Host project: `v3/Prose.Writer`
- Default dev URL: **http://localhost:5200** (the `http` launch profile — no self-signed-cert warning). HTTPS profile is `https://localhost:7200` if ever needed.
...
2. **Launch the host in the background** (pin the http profile so the URL is deterministic):
   ...
   ```
   Run it long-lived/backgrounded. If a current build exists, add `--no-build` to start faster.
...
5. Tell the user it's up at http://localhost:5200 and that the host keeps running in the background (stop it with `Get-Process Prose.Writer | Stop-Process -Force`).
