...
The law lives in `docs/LOGIC.md`. This is the runbook. **Never launch votes, panels,
or reviews from this runbook** — if the user wants a score, they will say so explicitly.
...
- **HARD, ABSOLUTE (2026-08-22): nothing reaches the database except through Prose.Hub — reads
  ...
  — it returns per-beat id/title/text in true reading order through the Hub, plus the beat's
  authoritative POV from `BeatEntityPresence` (never inferred from prose; see the doc comment on
  that command for the 2026-08-10 VIGL multi-POV misattribution it exists to prevent). Range-scope
  it (`--from`/`--to`) so each audit agent reads its own slice.
...
3. **VERIFY QUOTE GROUNDING (mechanical, before triage — docs/LOGIC.md §SS-LOGIC-4a).**
   ...
   Any Fail = that finding is misattributed or fabricated — drop it from triage and, if it seems
   worth chasing, re-read the actual beat yourself before deciding. This is cheap and mechanical;
   run it on every quoted finding, not just the ones that feel off.
...
8. **CONVERGENCE (docs/LOGIC.md §9) — replaces "run it again."** Do not decide "is this book
   ...
   campaign). It reports one of three things:
