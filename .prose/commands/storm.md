# STORM — resume point (GOSPEL glossary + corpus-wide publish-readiness pass)
...
**Blocked, now possibly unblocked by the MCP restart the user just did**: `switch_universe`
...
1. `list_universes` — confirm `gospel` is visible. If yes:
2. `switch_universe({slug: "gospel"})`.
3. Upsert all 87 deduplicated GOSPEL glossary terms below via `upsert_glossary_term`
   (batch in parallel calls, same pattern used for the 34 GLMZ + 34 SCRY additions earlier
