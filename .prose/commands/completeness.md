...
Usage: `/completeness [--fresh]`. `--fresh` re-runs coordination first (no LLM); default reads the
latest `reports/coordination/*.json`.
...
exceptions.** This runbook's steps 2 and 3 below previously instructed running raw `sqlcmd` SELECTs
...
5. **Render ONE page.** Design it deliberately, then write and publish it. Requirements:
   ...
   - State badge: aligned (100% covered, blueprint present); gap (coverage <100% or a
   ...
   - Theme-aware (light + dark), responsive, self-contained (no external assets).
...
6. **Summarize in chat** (below the page link): count of aligned vs gapped books, any flat
...
- To close gaps (not this runbook's job — it only reports): `prose --backfill-meaning` for
