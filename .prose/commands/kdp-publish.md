...
Usage: `/kdp-publish [--dry-run] [CODE1,CODE2,... to restrict to specific books]`
...
externally-visible change on the live Amazon KDP dashboard and burns real LLM API usage), then
...
`KdpOperatorService` drives an LLM tool-use agent loop against a live browser pane per book.
It is real, it costs real tokens, and a past run (2026-08-16) failed 100% across 19 books in ~4
seconds each because the account had hit its usage cap — check whether that or any
...
just those codes before proceeding — ignore the restriction if it names a code that isn't in
...
— do not launch anything), ask the user to confirm the exact list of codes about to be run.
This is a hard-to-reverse, externally-visible, real-money-adjacent action — do not skip the
confirmation gate even if the original ask was "automatically start and publish everything."
Include in the question: the codes, the count, and a one-line reminder that this costs live LLM
...
this is a safe no-op (nothing touches the live KDP dashboard before the first API call succeeds),
