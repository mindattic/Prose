# /review-node — economical node evaluation

Usage: `/review-node [slug-or-id]`; defaults to the most-recently-edited node.
...
> The 0–100 gates are retired; scores this runbook produces are display-only history.**

One call that grades a node and tells you what to fix — cheaply. The default is a **sampled two-tier** run (NOT a 1024-persona census): a stratified panel casts score-only ballots, those ballots double as the segment study, and only the most informative handful get upgraded to readable prose.
...
A score needs precision, not volume: with population SD ≈ 8, a sample of ~120 gives a 95% CI of ±~1.5 — tight enough to catch the ±2–3 pt moves edits produce. A 1024-census buys ±0.5 at ~8× the API calls (the per-call node-input tokens dominate cost). And you only ever read a dozen reviews. So the default casts cheap **score-ballots** (just numbers + one gripe) wide, then writes **full prose** for only the most informative few.

## Steps when invoked

1. **Resolve the node.** If a slug/id was given, use it. Otherwise default to the most-recently-edited:
   `SELECT TOP 1 Id, Slug, Title FROM Nodes ORDER BY UpdatedAt DESC`. Echo which node you're evaluating.

2. **GOTCHA — never edit beats during a run.** The run exports the node text at its start; editing mid-run splits the panel across versions. If beats were just edited, confirm the edit is fully applied BEFORE starting.

3. **Run the default sampled pass** (one command — ballots + study + prose, all in one). Launch it long-lived/backgrounded; wait for the completion notification (don't poll):
   ```
   dotnet run --project <proj> --no-build -- --review-node --slug <slug>
   ```
   Knobs: `--ballots N` (default 120) sets the sample size; `--prose N` (default 10) sets how many full reviews to write. The command prints the node mean + SD + 95% CI, cluster count, and the full Pareto/contested/seam report verbatim.

4. **Pool + verify** (a SQL client of your choosing — Unicode-safe; newest ContentHash first). The CLI already prints the headline number, but confirm/expand:
   - Newest ContentHash + ballot count (expect ≈ `--ballots`).
   - **Pooled mean, SD, 95% CI** (`±1.96*SD/sqrt(n)`).
   - **Provider split** (per-provider means) — the temperament gap (~9–12 pts across providers) usually dwarfs any panel gap. Read the pooled mean.
   - Compare vs the node's prior-version means (other ContentHashes): did the change clear the ±CI band, or is it inside the noise?
...
- `--local` → run the ballots (and the synopsis) on a **local LLM** instead of the cloud panel — free, no API tokens. See "Local LLM mode" below. Applies to the default sampled path and `--by-act` only; `--census/--study/--group/--same-personas` always run cloud.
...
- `--same-personas` → re-run the exact personas from the node's last batch (before/after focus group).
...
# Ollama defaults num_ctx to 2048, which TRUNCATES a node. Bake a real context window:
#   Modelfile:
...
**Run:**
```
dotnet run --project <proj> --no-build -- --review-node --slug <slug> --local
```

**Caveats — read these before trusting the number:**
- **No model/temperament diversity** — one local model = one temperament. Diversity is
  persona + psychometric only. Absolute calibration differs from the cloud panel mean.
- **Separate baseline.** Local ballots are stamped `ProviderId = 'local'`. Do NOT compare a
  local node mean against historical cloud means — start a fresh local baseline.
- **Speed.** Prefill (whole node in) dominates; at concurrency 2 a 20-ballot pass is a few
  minutes. Free, not instant. Big nodes still auto-route to the segmented path.
- If Ollama isn't running the run **fails fast** with an actionable error — it never silently
  falls back to the cloud.

## Reference: pooled-stats SQL skeleton
```
...
- Node headline score = mean of the ballots; per-beat % = mean of the ballots' micro-scores mapped positional→SortKey order.
