...
async function generateDocs(target) {
  ...
  for (let i = 0; i < needed; i += BATCH) {
    ...
    const system = `You generate worldbuilding documents for a near-future world called GLMZ (Great Lakes megacity, corporate sovereignty, neural interfaces ubiquitous, tiered citizenship). Return a JSON array of ${count} entries. Each must have: file_name (snake_case, unique), title, category ("${cat}"), body (3-6 paragraphs of rich worldbuilding — specific details, named places, implications for daily life), line_count (int, approximate), headings (array). These are reference documents that flesh out the world. Be specific, grounded, and avoid generic near-future cliches.`;
...
async function generateFactions(target) {
  ...
  for (let i = 0; i < needed; i += BATCH) {
    ...
    const system = `You generate faction entries for a near-future world called GLMZ (Great Lakes megacity corridor, corporate sovereignty, neural augmentation is ubiquitous, tiered citizenship). Return a JSON array of ${count} entries. Each must have: type ("faction"), name, aliases (array), motto, description (2-3 paragraphs — origins, current state, what makes them distinctive), ideology (1 paragraph), territory (where they operate), leadership (key figures or structure), methods (array of strings — how they operate), resources (array of strings — what they have access to), relationships (array of {faction, stance, description}), story_hooks (array). Generate a ${fType} type faction. Reference known districts: ${districts.slice(0,10).join(', ')} and CorpoNations: ${corps.slice(0,10).join(', ')}. Be specific and avoid generic near-future cliches.`;
