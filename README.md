...
Live at **[prose.azurewebsites.net](https://prose.azurewebsites.net/)**.
...
```
CLI (dotnet run --project v3/Prose.Cli -- <args>)
  │
  ▼
Blazor Server hosts  (v3/Prose.Writer/ — writing/recording UI, v3/Prose.Codex/ — canon/world-health/reports UI)
  │  Web UI (Razor components); the old single v3/Prose.Blazor/ host was split into these two 2026-07-06
  │
  ▼
...
| `PacingService` | BREATHE / FLOW / TIGHTEN / STRIKE / SETTLE prose rhythm | Position in book + BeatGoal keywords; Combat forces STRIKE |
...
| `ConsequenceEngine` | Cross-book persistent consequences (contract outcomes, faction burns) | `CharactersInScene.Count > 0` |
...
| `NarrativeSummaryService` | Rolling compressed scene memory — last 10 beats for long-book coherence | `BeatContext.NodeId != Guid.Empty` |
...
`Prose.Mcp` exposes 290+ `[McpServerTool]` methods across 34 tool-class files. The complete per-tool reference is [docs/MCP_TOOLS.md](docs/MCP_TOOLS.md), generated by reflection and auto-regenerated by the pre-commit hook.
...
All ~152 handlers are dispatched from `v3/Prose.Cli/Program.cs`.
...
Flags: `--local` (use stored `LocalLlmBaseUrl`), `--local-url <url>` (override endpoint), `--local-model <tag>` (override model), `--local-key <key>` (override bearer token). All overrides are ephemeral — they never mutate `appsettings.json`. The full enrichment pipeline (X-Ray context, BeatModeDetector, PacingService, book-bible injection) applies regardless of endpoint.
...
For book lists, scores, entity counts, and other read-only lookups, query the DB directly — returns in under a second:
...
| `Nodes` / `Nodes_History` | System-versioned book metadata + score |
...
| `VoiceChangeLog` | Proposed voice-rule changes harvested from winning books |
...
One-time, right after cloning — wires up the repo's git hooks (`.githooks/`), which are NOT
picked up automatically by a fresh clone (`.git/config` is never committed):
...
# temporal SYSTEM_VERSIONING). The standalone ApplyMigrations console app this used
# to be is gone — its job now lives in Prose.Cli. Idempotent; safe to re-run.
dotnet run --project Prose.Cli -- --migrate-sql --schema

# Optional: a handful of older, pre-EF-migration schema deltas and small data
# inserts still live as raw .sql under Prose.Core/Data/Sql/, run individually via
# `prose --seed <name>` (see SqlSeedService.Seeds for the registered names — most
# are superseded by the EF migrations above and rarely needed on a fresh DB).
# NOTE: not every file under Data/Sql/ is registered there yet (e.g. some
# universe-seed scripts) — if a universe/canon row you expect is missing, check
# Data/Sql/ for a same-named script and apply it by hand with sqlcmd.

dotnet run --project Prose.Writer
...
```powershell
dotnet test v3/Prose.UnitTests   # NUnit + bUnit
...
All optional — the app starts and most features work without any of them (credentials fall
back through `MindAttic.Vault` / DB-stored settings; see `SettingsService.ResolveApiKey`). Set
only what a feature you're using actually needs.
...
| Data paths | `PROSE_DATA_ROOT`, `PROSE_MUTABLE_DATA_ROOT` — override where canon/exports live; auto-detected otherwise |
| Universe scope | `PROSE_UNIVERSE` — default universe slug for CLI commands that need one |
| Local TTS | `PROSE_PYTHON`, `PROSE_ENG_SCRIPT`, `PROSE_ENG_VOICE`, `PROSE_CA_BUNDLE`, `PROSE_PIPER_EXE`, `PROSE_PIPER_MODEL` — see the `tools/chatterbox`/`tools/kokoro` note above; local TTS setup is currently incomplete |

`RunPod`/GPU routing uses a different variable — see the "GPU Entity Scoring" section below.
...
The first documented parallel-writing experiment (NxR book) ran Claude Opus vs. Qwen2.5:32b on the same book bible with the `--local-url` / `--local-model` flags. Both writers received identical X-Ray context, BeatModeDetector classification, PacingService rhythm, and book-bible injection. See `docs/strands/NxR.md §9`.
...
Every book follows this sequence (codified in `CLAUDE.md` and `docs/BIBLE.md §10`):
...
3. **Book structure.** Create a book node (`kind=book`) plus chapter sub-nodes (`kind=chapter`) parented to the book; the 14-beat authorial spine becomes the book node's `seed` text.
4. **Prose draft.** `ProseWriterRouter` is the entry point (SS-A16). It classifies beat mode, assigns pacing rhythm, injects structural role, loads active plant/payoff pairs, audits gateway/sequel commandments, and routes to `BeatGeneratorService` while logging per-book service coverage. Each beat's prompt is grounded by an X-Ray context block (`SceneContextAssembler`).
...
6. **Review (dual, mandatory).** A structural pre-flight runs first (`StructuralDiagnosticService`), then N Legion personas cast scored ballots (`NodeReviewService`). Per-book standalone must clear ≥82%; cumulative prefix must hold ≥85%.
...
> ~214 services in `v3/Prose.Core/Services/`, grouped by subsystem.
...
| Service | Role |
|---|---|
| `ProseWriterRouter` | **Sole prose entry point (SS-A16)** — coordinates all services below and logs per-book coverage |
| `DocContextService` / `DocContextStack` | Doc Context Stack — rotating cast of pertinent canon `.md` files (always core + book bible + topic docs triggered by keyword/embedding); gated by `SettingsService.DocContextEnabled` (default off) |
...
| `NodeBibleService` | Generates/manages per-book bibles (authorial spine, beat plan, synopsis) |
...
| `BookAuditService` | Gateway or Sequel commandment auditing (7 commandments each) |
| `PlantPayoffService` | Plant/payoff pair registry; seeds active plants into prose context; orphan-audit on book completion |
...
| `ArcTrackerService` | Tracks per-book five-act position and beat-level progression |
...
| Service | Role |
|---|---|
| `VoiceHarvestService` | Mines author edits + directives from winning books (≥80%) into proposed voice rules |
...
| Service | Role |
|---|---|
| `ManuscriptExportService` | All-in-one book export — EPUB 3 + PDF (QuestPDF, 6"×9" KDP trim) + Markdown into one `{Title}/V{N}/` dir |
...
The **Operator** (`v3/Prose.Core/Services/Operator/`) runs a Claude tool-use loop that writes chapters end-to-end by calling the world directly.
...
| `standard` | ~15 (−60%) | mid-tier defaults | the per-book standalone gate (≥82%) |
...
Live at **[prose.azurewebsites.net](https://prose.azurewebsites.net/)** · local dev at `https://localhost:7103/`
...
|---|---|---|
| `/writer` | Writer | Book picker — the launch pad for writing |
| `/write/{BookId}/{ChapterId}` | Write | Beat-level prose editor — write, expand, reflow, review beats in one screen |
| `/node/{IdOrSlug}` | Book | Single book workbench — beats list, status, review scores, export controls |
| `/nodes` | Books | Full book index grouped by kind |
...
`Prose.Mcp` exposes **290+ `[McpServerTool]` methods** across 34 tool-class files. The complete per-tool reference is [docs/MCP_TOOLS.md](docs/MCP_TOOLS.md), generated by reflection over the live attributes and auto-regenerated by the pre-commit hook.

```powershell
dotnet run --project v3/Prose.Mcp -- --export-tools docs/MCP_TOOLS.md
```

A representative sample of the 34 tool-class files — see [docs/MCP_TOOLS.md](docs/MCP_TOOLS.md) for the full, always-current list:
...
| `Tools.StoryAudit.cs` | `audit_book_commandments`, `get_book_spine` |
| `Tools.Swain.cs` | `swain_audit`, `swain_audit_all`, `swain_repair` — Scene/Sequel doctrine classification + auto-splice |
...
| `Tools.LoreTriples.cs` | Continuity-claim extraction; list/resolve/apply contradictions; `append_book_amendment` |
...
| Agents | `Prose.Mcp` — MCP server exposing canon to Claude clients |
...
│   ├── Prose.Cli/       # Standalone CLI console app — includes --migrate-sql (schema bootstrap)
...
│   ├── Prose.Mcp/       # Model Context Protocol server (~195+ tools)
│   └── Prose.UnitTests/ # NUnit + bUnit tests
├── tools/
...
├── docs/                        # Codex docs — BIBLE.md, USER_STORIES.md, rfc/
├── engine_data/                 # Canon entity seed/export mirror (SQL is the live read path)
└── cypress/                     # Cypress end-to-end tests
```

> **tools/chatterbox and tools/kokoro:** as of this writing both directories contain only a
> committed Python `.venv/` with no `requirements.txt` and no setup script — and that `.venv`
> is itself broken (its activation scripts hardcode a pre-rename path from before the
> StreetSamurai → Prose rename). Local TTS via these tools currently needs a from-scratch
> Python environment rebuilt by hand; ElevenLabs TTS (cloud) works out of the box and needs no
> local setup. Flagged for cleanup, not yet fixed.

---

## Database migrations

Raw T-SQL files live under `v3/Prose.Core/Data/Sql/`. Most are historical, pre-EF-migration
schema deltas superseded by `v3/Prose.Core/Migrations/` — run individually via
`prose --seed <name>` (see `SqlSeedService.Seeds` for registered names) only if a specific
older environment needs them. A few small data-only scripts (e.g. universe seed inserts) are
not yet registered anywhere and would need to be applied by hand with `sqlcmd` if a fresh DB
is missing rows you expect.
...
Live site: **prose.azurewebsites.net** on Azure App Service against Azure SQL (Serverless GP). CI/CD runs `build → migrate → deploy` on every push to master via GitHub Actions. Full provisioning guide: [`infra/README.md`](infra/README.md).
...
Ten invention disclosures documenting novel systems in the Prose engine. All are pre-filing confidential documents; formal claim drafting to be conducted by qualified patent counsel.
...
**Books complete (≥82% standalone):** BCODA (88.8% cumulative, 16 chapters), ATTE (89.2%), VATD (88.1%), DWIACE (90.95%), SPRW (87.0%), SRZR (86.6%), TEST (86.7%), MxG (86.4%), UNDR (83.2%). MNEMO in progress (84.0%).

**DB:** 14,500+ active entities · 11,700+ enabled beats · books across 4 universes (GLMZ cyberpunk, SCRY fantasy/steampunk, SOURCE citation-grounded nonfiction, EPIC).

**MCP:** 290+ tools across 34 tool-class files.
