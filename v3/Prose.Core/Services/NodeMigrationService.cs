...
public class NodeMigrationService
{
    ...
    public NodeMigrationService(IDbContextFactory<ProseDbContext> dbFactory, ILogger<NodeMigrationService> log)
    {
    ...
    private async Task<int> MigrateBooksAsync(ProseDbContext db, CancellationToken ct)
    {
    ...
    private async Task<int> MigrateChaptersAsync(ProseDbContext db, CancellationToken ct)
    {
    ...
    private async Task<(int beatsAdded, int junctionsAdded)> MigrateChapterBeatsAsync(ProseDbContext db, CancellationToken ct)
    {
    ...
    private async Task<(int episodesAdded, Dictionary<string, Guid> chapterIdToNode)> MigrateEpisodesAsync(ProseDbContext db, CancellationToken ct)
    {
        var rows = await db.Database
    ...
    private async Task<(int standaloneAdded, int junctionsAdded)> MigrateEpisodeBeatsAsync(ProseDbContext db, CancellationToken ct)
    {
        var rows = await db.Database
