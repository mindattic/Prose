...
/// segment/silence/encode assembly in <see cref="NodeWorkbenchService.ExportAudiobookAsync"/>
...
public sealed class PythonTtsService : ILocalTtsEngine
{
    ...
    public PythonTtsService(string engine, ILogger? log = null)
    {
        ...
        python = Environment.GetEnvironmentVariable("PROSE_PYTHON");
    ...
    /// <summary>tools\corp-ca-bundle.pem (env PROSE_CA_BUNDLE overrides), or null if absent.</summary>
    private static string? ResolveCaBundle()
    {
        var env = Environment.GetEnvironmentVariable("PROSE_CA_BUNDLE");
