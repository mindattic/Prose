...
/// does the same. <see cref="UniverseContext"/> also reads the <c>PROSE_UNIVERSE</c> environment
...
public static class UniverseBootstrap
{
    ...
    /// <summary>
    /// The requested slug from the <c>--universe</c> flag or the <c>PROSE_UNIVERSE</c> env var, mapped
    ...
    {
        var slug = RequestedSlug ?? Environment.GetEnvironmentVariable("PROSE_UNIVERSE");
        return slug?.Trim().ToLowerInvariant() switch
        {
            "glmz" => Prose.Core.Data.Entities.Universe.GlmzId,
            "scry" or "fantasy-steampunk" or "fantasy" or "steampunk" => Prose.Core.Data.Entities.Universe.FantasyId,
            "nonfiction" or "source" or "gspl" => Prose.Core.Data.Entities.Universe.NonfictionId,
            _ => null,
        };
    }
}
...
/// <summary>
/// Ambient hook the <see cref="ProseDbContext"/> reads in its global query filters and
...
public static class UniverseScope
{
    ...
    /// CLI <c>--universe</c>/<c>PROSE_UNIVERSE</c> well-known fallback so CLI processes scope correctly
...
public sealed class UniverseContext : IUniverseContext
{
    ...
    private void EnsureLoaded()
    {
        lock (gate)
        {
            ...
            // Per-process selection: --universe flag (UniverseBootstrap) or PROSE_UNIVERSE env var
