...
/// <summary>
/// SQLite-in-memory <see cref="IDbContextFactory{ProseDbContext}"/> for unit
...
{
    private static readonly ConcurrentDictionary<string, IDbContextFactory<ProseDbContext>> Cache = new();

    public static IDbContextFactory<ProseDbContext> For(IPathProvider paths, string _entityType)
    {
    ...
    private sealed class SqliteInMemoryFactory : IDbContextFactory<ProseDbContext>, IDisposable
    {
        ...
        private static void SeedFromJson(ProseDbContext db, string engineDataDir)
        {
