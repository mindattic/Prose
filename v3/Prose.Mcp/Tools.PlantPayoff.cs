    ...
    HubInvoker hub)
{
    ...
    public Task<string> get_plant_payoffs(
        [Description("Node id (GUID) or slug.")] string nodeIdOrSlug) =>
        hub.InvokeAsync(nameof(PlantPayoffTools), nameof(get_plant_payoffsImpl), new { nodeIdOrSlug });
    ...
    public async Task<string> get_plant_payoffsImpl(string nodeIdOrSlug)
    {
    ...
    public Task<string> register_plant_payoff(
        [Description("Node id (GUID) or slug.")] string nodeIdOrSlug,
        ...
        hub.InvokeAsync(nameof(PlantPayoffTools), nameof(register_plant_payoffImpl), new { nodeIdOrSlug, plantDescription, payoffDescription, category, plantBeatId, payoffBeatId });

    /// <summary>The real logic — runs inside the Hub's process via ToolDispatch reflection, never called directly by this process.</summary>
    public async Task<string> register_plant_payoffImpl(
        string nodeIdOrSlug,
        string plantDescription,
        string payoffDescription,
        string category = "detail",
        string? plantBeatId = null,
        string? payoffBeatId = null)
    {
        var nodeId = await ResolveNodeAsync(nodeIdOrSlug);
        if (nodeId == null)
            return JsonSerializer.Serialize(new { error = "node_not_found", nodeIdOrSlug }, JsonOpts);
    ...
    public Task<string> link_plant_beat(
        ...
        hub.InvokeAsync(nameof(PlantPayoffTools), nameof(link_plant_beatImpl), new { plantPayoffId, beatId });

    /// <summary>The real logic — runs inside the Hub's process via ToolDispatch reflection, never called directly by this process.</summary>
    public async Task<string> link_plant_beatImpl(string plantPayoffId, string beatId)
    {
    ...
    public Task<string> link_payoff_beat(
        [Description("PlantPayoff id (GUID) from register_plant_payoff.")] string plantPayoffId,
        [Description("Beat GUID containing the payoff.")] string beatId) =>
        hub.InvokeAsync(nameof(PlantPayoffTools), nameof(link_payoff_beatImpl), new { plantPayoffId, beatId });

    /// <summary>The real logic — runs inside the Hub's process via ToolDispatch reflection, never called directly by this process.</summary>
    public async Task<string> link_payoff_beatImpl(string plantPayoffId, string beatId)
    {
        if (!Guid.TryParse(plantPayoffId, out var ppId) || !Guid.TryParse(beatId, out var bId))
            return JsonSerializer.Serialize(new { error = "invalid_guid" }, JsonOpts);
        try
    ...
    public Task<string> set_plant_transparency(
        ...
        hub.InvokeAsync(nameof(PlantPayoffTools), nameof(set_plant_transparencyImpl), new { plantPayoffId, isTransparent, note });

    /// <summary>The real logic — runs inside the Hub's process via ToolDispatch reflection, never called directly by this process.</summary>
    public async Task<string> set_plant_transparencyImpl(string plantPayoffId, bool isTransparent, string? note = null)
    {
