...
public class AuditDeterministicRuleTests
{
    ...
    {
        Assert.That(CraftNativeRules.InteriorityDensityRule.CountItalics("*One.* plain *Two.*"), Is.EqualTo(2));
        Assert.That(CraftNativeRules.InteriorityDensityRule.CountItalics("**bold** not italics"), Is.EqualTo(0));
        Assert.That(CraftNativeRules.InteriorityDensityRule.CountItalics("no markers at all"), Is.EqualTo(0));
    }
    ...
    public async Task InteriorityDensity_AtTrnyShippedRate_FiresModerate()
    {
        ...
        var verdicts = await new CraftNativeRules.InteriorityDensityRule().EvaluateAsync(Ctx(beats), default);
    ...
    public void FindTics_RetiredCognitiveFraming_IsFlagged(string prose)
        => Assert.That(CraftNativeRules.RetiredTicRule.FindTics(prose), Is.Not.Empty);
    ...
    public void FindTics_LiteralDiegeticBookkeeping_IsNotFlagged(string prose)
        => Assert.That(CraftNativeRules.RetiredTicRule.FindTics(prose), Is.Empty,
    ...
    public async Task RetiredTic_CitesTheBeatAndQuotesSurroundingText()
    {
        ...
        var verdicts = await new CraftNativeRules.RetiredTicRule().EvaluateAsync(Ctx(beats), default);
