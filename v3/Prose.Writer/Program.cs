...
using Prose.Writer.Components;
using Prose.Writer.Services;
...
// Writer lean profile: all singletons (lazy) but NO background hosted services —
// home-stats refresh, global-search warmup, audio reconciliation, world tick, and
// the continuity long sweep run in the Codex host only. See AddWriterServices docs.
builder.Services.AddWriterServices();
...
// ProseAuthDbContext. Do NOT also call AddAuthentication/AddCookie/AddAuthorization/
...
builder.Services.AddMindAtticAuthentication<ProseAuthDbContext>(
    ...
    o =>
    {
        o.AppName = "Prose";                          // per-app Data Protection trust boundary
        ...
        // DEV: the library persists the key ring to %APPDATA%\MindAttic\DataProtection\Prose.
    });
...
builder.Services.AddScoped<Prose.Shared.Services.TabService>();
...
// early reads would run unscoped. Honors a --universe flag / PROSE_UNIVERSE on the host.
app.Services.GetRequiredService<IUniverseContext>();

// Auth startup orchestration (migrate → import → seed) runs in the CODEX host
// only — one process owns schema/seed work against the shared auth tables. The
// Writer host reads the already-migrated schema and the shared auth cookie.
...
_ = Task.Run(() =>
{
    try
    {
        _ = app.Services.GetRequiredService<Prose.Core.Services.ContinuousQualityService>();
    }
    ...
    try
    {
        _ = app.Services.GetRequiredService<Prose.Core.Services.BeatStateExtractor>();
    }
...
app.MapRazorComponents<App>()
    ...
    .AddAdditionalAssemblies(typeof(Prose.Shared.Components.Pages.Login).Assembly);
...
static async Task<string?> ResolveSlugAsync(
    Microsoft.EntityFrameworkCore.IDbContextFactory<Prose.Core.Data.ProseDbContext> dbFactory,
    Guid episodeId)
{
...
app.MapGet("/api/episodes/{episodeId:guid}/audio/{index:int}", async (
    Guid episodeId,
    int index,
    Prose.Core.Services.EpisodeAudioService audio,
    Microsoft.EntityFrameworkCore.IDbContextFactory<Prose.Core.Data.ProseDbContext> dbFactory,
    HttpContext ctx) =>
{
...
app.MapGet("/api/nodes/{nodeId:guid}/beat/{beatId:guid}/audio", async (
    Guid nodeId,
    Guid beatId,
    Prose.Core.Interfaces.IAudioStore audioStore,
    Microsoft.EntityFrameworkCore.IDbContextFactory<Prose.Core.Data.ProseDbContext> dbFactory) =>
{
    await using var db = await dbFactory.CreateDbContextAsync();
...
// Distributed worker REST API (/api/worker/*) lives in the CODEX host only —
// the coordinator process must be the single writer to review tables.

Log.Information("Prose Writer host started");
