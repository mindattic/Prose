...
public static class AuditNodeCli
{
    public static async Task<int> RunAsync(IServiceProvider sp, string[] args)
    {
        ...
        if (slug == null) { Console.Error.WriteLine("Usage: ss --audit-node --slug <slug> [--deep] [--model <id>] [--out <path>]"); return 2; }
        ...
        W($"_Generated by `ss --audit-node`{(deep ? $", deep tier, model={model ?? "default"}" : ", free tier")}._");
        W();
        ...
        if (totEq == 0)          gaps.Add("EmotionalScore never populated (run --audit-node --deep, or ss --examine-emotion).");
        ...
        if (deep)
        {
            ...
            try
            {
                foreach (var ch in chapters)
                {
                    ...
                    var (dgExit, dgOut)   = await Capture(() => DiagnoseNodeCli.RunAsync(["--diagnose-node", "--slug", ch.Slug, "--json"], sp));
