...
public static class RetireRecordsBlobsCli
{
    private sealed record TypeDef(
        string EntityType,
        Func<StreetSamuraiDbContext, CancellationToken, Task<int>> Rebuild,
        Func<StreetSamuraiDbContext, HashSet<Guid>> GetSpineIds);
    ...
    private static async Task ShowStatusAsync(IDbContextFactory<StreetSamuraiDbContext> dbFactory)
    {
    ...
    private static async Task<int> RunRebuildAsync(IDbContextFactory<StreetSamuraiDbContext> dbFactory)
    {
        int failures = 0;
    ...
    private static async Task<int> RebuildCharactersAsync(StreetSamuraiDbContext db)
    {
    ...
    private static async Task<int> RunValidateAsync(IDbContextFactory<StreetSamuraiDbContext> dbFactory)
    {
        await using var db = await dbFactory.CreateDbContextAsync();
        ...
        foreach (var t in Types)
        {
            var blobEntityIds = await db.Entities.AsNoTracking()
                .Where(e => e.EntityType == t.EntityType && e.IsActive
                    && db.Records.Any(r => r.EntityId == e.Id))
                .Select(e => e.Id)
                .ToListAsync();
        ...
        // character spine check
        {
            int charOrphans = await db.Entities.AsNoTracking()
                .CountAsync(e => e.EntityType == "character" && e.IsActive
                    && db.Records.Any(r => r.EntityId == e.Id)
                    && !db.Characters.Any(c => c.Id == e.Id));
    ...
    private static async Task<int> RunApplyAsync(IDbContextFactory<StreetSamuraiDbContext> dbFactory)
    {
        await using var db = await dbFactory.CreateDbContextAsync();
