...
/// <summary>
/// <c>ss --duplicate-node (--id &lt;guid|prefix&gt; | --slug &lt;slug&gt;) --title "New Title"</c>
...
public static class DuplicateNodeCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider services)
    {
        ...
        {
            Console.Error.WriteLine("[duplicate-node] One of --id or --slug is required.");
            return 1;
        }
        if (string.IsNullOrWhiteSpace(title))
        {
            Console.Error.WriteLine("[duplicate-node] --title \"New Title\" is required.");
            return 1;
        }
        ...
        await using (var db = await dbFactory.CreateDbContextAsync())
        {
            ...
            if (node == null) { Console.Error.WriteLine("[duplicate-node] Source node not found."); return 1; }
            sourceId = node.Id; sourceTitle = node.Title;
        }

        Console.WriteLine($"[duplicate-node] Duplicating \"{sourceTitle}\" -> \"{title}\"…");
        try
        {
            var (newId, newSlug) = await workbench.DuplicateNodeAsync(sourceId, title!);
            Console.WriteLine($"[duplicate-node] OK");
        ...
        catch (Exception ex) { Console.Error.WriteLine($"[duplicate-node] Failed: {ex.Message}"); return 1; }
    }
}
