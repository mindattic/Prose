...
/// <summary>
/// ss --plant-audit  --slug &lt;strandSlug&gt; [--json]
///   Audit all plant/payoff pairs: orphaned plants, transparency violations, coverage.
///
/// ss --list-plants  --slug &lt;strandSlug&gt; [--json]
///   List all registered plant/payoff pairs for a strand.
///
/// ss --add-plant  --slug &lt;strandSlug&gt;
...
public static class PlantPayoffCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider services)
    {
        ...
        if (slug == null) { Console.Error.WriteLine("Usage: ss --plant-audit|--list-plants|--add-plant --slug <strand> [options]"); return 2; }
        ...
        {
            var pairs = await svc.GetByStrandAsync(strand.Id);
            if (jsonMode)
            {
            ...
            Console.WriteLine($"Plants/Payoffs for '{strand.Title}' ({pairs.Count} pairs)\n");
        ...
        if (args.Contains("--add-plant"))
        {
            ...
            {
                Console.Error.WriteLine("Usage: ss --add-plant --slug <strand> --plant \"what is seeded\" --payoff \"what re-readers get\" [--cat detail]");
                return 2;
            }

            var pp = await svc.RegisterAsync(strand.Id, plant, payoff, cat);
            if (jsonMode)
            {
        ...
        if (!jsonMode) Console.WriteLine($"Auditing plant/payoff pairs for '{strand.Title}'…\n");

        var audit = await svc.AuditAsync(strand.Id);
