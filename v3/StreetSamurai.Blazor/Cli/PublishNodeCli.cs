...
/// <summary>
/// <c>ss --publish-node</c> — stitch an existing node's recorded beats into
...
public static class PublishNodeCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider services)
    {
        ...
        {
            Console.Error.WriteLine("[publish-node] One of --id or --slug is required.");
            Console.Error.WriteLine("Usage: ss --publish-node (--id <guid|prefix> | --slug <slug>)");
            return 1;
        }
        ...
        await using (var db = await dbFactory.CreateDbContextAsync())
        {
            ...
            else
            {
                ...
                if (matches.Count > 1)
                {
                    Console.Error.WriteLine($"[publish-node] Id prefix '{id}' is ambiguous. Use a longer prefix or the full id.");
                    return 1;
                }
            ...
            if (node == null)
            {
                Console.Error.WriteLine($"[publish-node] No node found for {(slug != null ? $"slug '{slug}'" : $"id '{id}'")}.");
                return 1;
            }
            nodeId = node.Id; nodeSlug = node.Slug; nodeTitle = node.Title;
        }

        Console.WriteLine($"[publish-node] Publishing:");
        ...
        {
            Console.Error.WriteLine($"[publish-node] Publish failed: {ex.Message}");
            return 1;
        }

        if (rel == null)
        {
            Console.Error.WriteLine("[publish-node] Nothing to publish — record beats first (or beats are mixed-format).");
            return 1;
        }
        ...
        await using (var db = await dbFactory.CreateDbContextAsync())
        {
            ...
            Console.WriteLine($"[publish-node] Combined internal path: {rel}");
            if (pub != null)
            {
                Console.WriteLine($"[publish-node] Publication: {pub.Status}, {pub.Format}, {pub.BeatCount} beats, {pub.ByteSize:N0} bytes");
                Console.WriteLine($"[publish-node] Exported to: {pub.Path}");
            }
            Console.WriteLine($"[publish-node] Audio-event ledger rows for this node: {eventCount}");
        }
        Console.WriteLine("[publish-node] Done.");
        return 0;
    }
}
