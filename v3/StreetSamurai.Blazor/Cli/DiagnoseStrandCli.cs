...
/// </summary>
public static class DiagnoseStrandCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider services)
    {
        ...
        if (slug == null)
        {
            Console.Error.WriteLine("Usage: ss --diagnose-strand --slug <strandSlug> [--json]");
            return 2;
        }
        ...
        if (!jsonMode)
            Console.WriteLine($"Diagnosing '{strand.Title}' — running 12 structural checks in parallel…\n");

        var result = await svc.DiagnoseStrandAsync(strand.Id);
