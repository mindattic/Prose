...
/// <summary>
/// <c>ss --rebeat-strand (--slug &lt;s&gt; | --id &lt;guid|prefix&gt;) [--apply]</c> —
/// rebuild a strand's beats to the codified beat doctrine via LLM re-segmentation
...
/// strand. <c>--apply</c> exports a markdown backup, then (only if the guard
/// passes) replaces the beats and assigns gaps.
///
/// <c>--all</c> rebuilds every strand whose beats violate the doctrine (skips the
/// already-conforming ones); still honors <c>--apply</c>.
/// </summary>
public static class RebeatStrandCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider services)
    {
        ...
        await using (var db = await dbFactory.CreateDbContextAsync())
        {
            if (all)
            {
                ...
                Console.WriteLine($"[rebeat] {targets.Count} strand(s) violate the beat doctrine.{(apply ? "" : "  (dry run)")}");
            }
            else
            {
                Strand? strand;
                if (!string.IsNullOrWhiteSpace(slug)) strand = await db.Strands.AsNoTracking().FirstOrDefaultAsync(s => s.Slug == slug);
                else if (Guid.TryParse(id, out var g)) strand = await db.Strands.AsNoTracking().FirstOrDefaultAsync(s => s.Id == g);
                else strand = await db.Strands.AsNoTracking().Where(s => s.Id.ToString().StartsWith(id!.ToLower())).Take(2).ToListAsync() switch
                { { Count: 1 } m => m[0], _ => null };
                if (strand == null) { Console.Error.WriteLine("[rebeat] Strand not found (or id prefix ambiguous)."); return 1; }
                targets.Add((strand.Id, strand.Title));
            }
        }
        ...
        Console.WriteLine(apply
            ...
            : $"[rebeat] Dry run complete for {targets.Count} strand(s). Add --apply to commit.");
        return 0;
    }

    /// <summary>
    /// Strands whose beats violate the doctrine: duplicate beats, no dialogue/paragraph
    /// formatting (zero newlines across beats), heavy over-fragmentation, or giant
    /// run-on beats. Conforming strands (formatted, no dups, sane sizing) are skipped.
    /// </summary>
    private static async Task<List<(Guid, string)>> FindViolatorsAsync(StreetSamuraiDbContext db)
    {
        var strandIds = await db.Strands.AsNoTracking().Select(s => new { s.Id, s.Title }).ToListAsync();
        var result = new List<(Guid, string)>();
        foreach (var s in strandIds)
        {
            var lens = await db.StrandBeats.AsNoTracking().Where(sb => sb.StrandId == s.Id)
