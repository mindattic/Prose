...
/// strand and emits a single consolidated scorecard. Book-aware: a book fans out into its
...
///   DEEP  (--deep)  — per-chapter examine-emotion + story-audit + diagnose-strand + check-fidelity.
...
/// to a markdown file.
/// </summary>
public static class AuditStrandCli
{
    public static async Task<int> RunAsync(IServiceProvider sp, string[] args)
    {
        ...
        if (slug == null) { Console.Error.WriteLine("Usage: ss --audit-strand --slug <slug> [--deep] [--model <id>] [--out <path>]"); return 2; }
        ...
        var root = await db.Strands.AsNoTracking()
            ...
            .Select(s => new { s.Id, s.Title, s.Slug, s.Score, s.PreviousStrandId, s.Seed })
            .FirstOrDefaultAsync();
        ...
        W($"# Strand Audit — {root.Title} ({root.Slug})");
        W($"_Generated by `ss --audit-strand`{(deep ? $", deep tier, model={model ?? "default"}" : ", free tier")}._");
        W();
        ...
        if (totEq == 0)          gaps.Add("EmotionalScore never populated (run --audit-strand --deep, or ss --examine-emotion).");
        ...
        var reviews = await db.StrandReviews.AsNoTracking()
            .Where(r => scopeIds.Contains(r.StrandId))
            .GroupBy(r => r.StrandId)
            .Select(g => new { StrandId = g.Key, Ballots = g.Count(), Avg = g.Average(x => x.Score), Flow = g.Average(x => (double?)x.FlowScore) })
            .ToListAsync();
        var titleById = chapters.ToDictionary(c => c.Id, c => c.Title);
        titleById[root.Id] = root.Title + " (book)";
        foreach (var r in reviews.OrderByDescending(x => x.StrandId == root.Id).ThenBy(x => titleById.GetValueOrDefault(x.StrandId)))
            W($"| {Trunc(titleById.GetValueOrDefault(r.StrandId, "?"), 30)} | {r.Ballots} | {r.Avg:F0} | {r.Flow:F0} |");
        W();
        var weakest = reviews.Where(r => r.StrandId != root.Id).OrderBy(r => r.Avg).Take(3)
            .Select(r => $"{titleById.GetValueOrDefault(r.StrandId)} ({r.Avg:F0}/{r.Flow:F0})").ToList();
        ...
        var coverage = await monitor.GetStrandCoverageAsync(root.Id);
        ...
        if (deep)
        {
            ...
            try
            {
                foreach (var ch in chapters)
                {
                    ...
                    var (dgExit, dgOut)   = await Capture(() => DiagnoseStrandCli.RunAsync(["--diagnose-strand", "--slug", ch.Slug, "--json"], sp));
        ...
        W($"- Strand score (clustered): {root.Score:F2}");
        W();
    ...
    private static async Task<Census> CensusChapterAsync(StreetSamuraiDbContext db, Guid chapterId)
    {
        var beats = await (from sb in db.StrandBeats.AsNoTracking()
                           ...
                           where sb.StrandId == chapterId && sb.IsEnabled
