...
/// <summary>
/// <c>ss (--publish-md | --publish-txt | --publish-pdf) (--id &lt;guid|prefix&gt; | --slug &lt;slug&gt;) [--author "Name"]</c>
/// — render a strand to Markdown, plain text, or PDF in the user's Downloads
/// folder. The headless twin of the writer page's Export dropdown items.
/// </summary>
public static class PublishManuscriptCli
{
    public enum Format { Markdown, Text, Pdf }
    ...
    public static async Task<int> RunAsync(string[] args, IServiceProvider services, Format format)
    {
        var (tag, ext) = format switch
        {
            ...
            Format.Text     => ("publish-txt", "plain text .txt"),
            _               => ("publish-pdf", "PDF"),
        };
        ...
        try
        {
            var path = format switch
            {
                ...
                Format.Text     => await export.ExportTextAsync(strandId, author),
