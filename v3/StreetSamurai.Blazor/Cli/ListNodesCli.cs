...
/// <summary>
/// <c>ss --list-nodes</c> — print every node as a table (or JSON). Headless
...
public static class ListNodesCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider services)
    {
        ...
        for (int i = 0; i < args.Length; i++)
        {
            switch (args[i])
            {
                ...
                case "--limit":
                    ...
                    else { Console.Error.WriteLine("[list-nodes] --limit needs a positive integer."); return 1; }
                    break;
        ...
        var nodes = await query
            .Select(s => new { s.Id, s.Title, s.Slug, s.Kind, s.Status, s.NodeCode, s.Score, s.UpdatedAt, BeatCount = s.NodeBeats.Count })
            .ToListAsync();

        // Word / page count — one join instead of N queries
        var ids = nodes.Select(s => s.Id).ToList();
        var charCounts = await db.NodeBeats
            .AsNoTracking()
        ...
        if (rows.Count == 0)
        {
            Console.WriteLine("[list-nodes] No nodes match.");
            return 0;
        }
        ...
        Console.WriteLine(new string('-', 110));
        Console.WriteLine($"[list-nodes] {rows.Count} node(s).");
        return 0;
    }
