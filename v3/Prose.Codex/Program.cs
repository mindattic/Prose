...
using Prose.Codex.Components;
using Prose.Codex.Services;
...
// All Prose services (repos, graph, LLM, TTS, etc.)
...
using (var scope = app.Services.CreateScope())
{
    ...
    {
        var authDb = sp.GetRequiredService<ProseAuthDbContext>();
        await authDb.Database.MigrateAsync();
    }
    var imported = await sp.GetRequiredService<Prose.Core.Services.AuthUserImportService>().ImportAsync();
...
app.MapGet("/api/worker/status", async (
    Prose.Core.Services.DistributedWorkerCoordinator coordinator,
...
app.MapPost("/api/worker/submit", async (
    Prose.Core.Services.WorkerResult result,
    Prose.Core.Services.DistributedWorkerCoordinator coordinator,
    IConfiguration cfg, HttpContext ctx) =>
{
    if (!WorkerAuthOk(ctx, cfg)) return Results.Unauthorized();
...
Log.Information("Prose Codex host started");
