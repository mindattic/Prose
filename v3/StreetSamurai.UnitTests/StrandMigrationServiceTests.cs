...
/// <summary>
/// Locks in the contract of <see cref="StrandMigrationService"/>:
/// <list type="bullet">
/// <item>Books / Chapters / ChapterBeats / Episodes / EpisodeBeats translate
///   to Strands / Beats / StrandBeats with parent-child wiring preserved.</item>
...
/// </summary>
[TestFixture]
public class StrandMigrationServiceTests
{
    private string tempRoot = "";
    private TestPathProviderWithRoot paths = null!;
    private IDbContextFactory<StreetSamuraiDbContext> dbFactory = null!;
    private StrandMigrationService svc = null!;

    [SetUp]
    public void SetUp()
    {
        tempRoot = Path.Combine(Path.GetTempPath(), "ss-strand-mig-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);
        paths = new TestPathProviderWithRoot(tempRoot);
        dbFactory = TestDbFactory.For(paths, "strand-migration");
        svc = new StrandMigrationService(dbFactory, NullLogger<StrandMigrationService>.Instance);
    }
    ...
    [Test]
    public async Task MigrateAll_OnFreshLegacyData_ProducesMatchingStrandRows()
    {
    ...
    public async Task MigrateAll_IsIdempotent_OnReRun()
    {
        ...
        Assert.That(await db.StrandBeats.CountAsync(), Is.EqualTo(6),  "Re-run must not duplicate junction rows");
    }
