...
PROMPTS = {
    "characters": """You are a character description engine for a cyberpunk worldbuilding project.
...
def run_describe(repo="characters", limit=None, dry_run=False, concurrency=None):
    """Main entry point."""
    asyncio.run(_run_describe_async(repo, limit, dry_run, concurrency))


async def _run_describe_async(repo="characters", limit=None, dry_run=False, concurrency=None):
    import anthropic

    # Determine which repos to process
    if repo == "all":
        repos = ["characters", "weaponry", "equipment", "apparel"]
    else:
        repos = [repo]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--repo", type=str, default="characters",
                        help="Repo to process: characters, weaponry, equipment, apparel, or 'all'")
