...
/// <summary>
/// Dump the StreetSamurai SQL Server database to a single re-runnable .sql file.
///
///   ss --sql-export --schema [--out path.sql]   schema-only DDL
///   ss --sql-export --data   [--out path.sql]   schema + INSERT statements
///
/// Output goes to ./streetsamurai-{schema|full}-{timestamp}.sql by default.
///
...
public static class SqlExportCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider sp)
    {
        ...
        var outPath = outIdx >= 0 && outIdx + 1 < args.Length
            ...
            : Path.Combine(Environment.CurrentDirectory,
                $"streetsamurai-{(schemaOnly ? "schema" : "full")}-{DateTime.Now:yyyyMMdd-HHmmss}.sql");

        using var scope = sp.CreateScope();
        var dbFactory = scope.ServiceProvider.GetRequiredService<IDbContextFactory<StreetSamuraiDbContext>>();
    ...
    private static async Task WriteHeaderAsync(StreamWriter w, SqlConnection conn, bool withData)
    {
        ...
        await w.WriteLineAsync("-- ============================================================");
        await w.WriteLineAsync($"-- StreetSamurai SQL export — {(withData ? "schema + data" : "schema only")}");
