...
public static class ReflowNodeCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider services)
    {
        ...
        {
            Console.Error.WriteLine("[reflow-book] One of --id or --slug is required.");
            return 1;
        }
        ...
        await using (var db = await dbFactory.CreateDbContextAsync())
        {
            ...
            if (node == null) { Console.Error.WriteLine("[reflow-book] Node not found."); return 1; }
            nodeId = node.Id; nodeTitle = node.Title;
        }

        Console.WriteLine($"[reflow-book] {(apply ? "APPLYING to" : "DRY-RUN on")} \"{nodeTitle}\"…");
        try
        {
