...
///
/// Dump a node bible VERBATIM — the exact <c>Nodes.NodeBible</c> text, byte for byte, with no
...
/// <c>--book-bible</c> (which GENERATES a fresh one via an LLM — destructive, not a dump) and the
/// MCP <c>get_book_bible</c> tool (which returns 100K+ chars into the caller's context). That left
/// no safe way to do the read-edit-write round trip <c>--set-book-bible --file</c> exists for, so a
...
/// </summary>
public static class GetBookBibleCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider services)
    {
        ...
        if (string.IsNullOrWhiteSpace(slug))
        {
            ...
            Console.Error.WriteLine("Usage: prose --get-book-bible --slug <slug|code|guid> [--out <path>]");
            return 2;
        }
        ...
        // SetBookBibleCli uses, so a dump and its matching push always target the same row.
        ...
        if (bible.Length == 0)
            Console.Error.WriteLine($"[get-book-bible] WARNING: '{node.Title}' ({node.Slug}) has an empty NodeBible.");
