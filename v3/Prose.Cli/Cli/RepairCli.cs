...
public static class RepairCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider sp)
    {
        ...
        Console.WriteLine("=== Prose story repair ===");
        ...
        var ammoLinker = sp.GetRequiredService<Prose.Core.Services.AmmunitionLinkerService>();
        await ammoLinker.EnsureWeaponSpecsSchemaAsync(ct);
        await ammoLinker.EnsureGearEntityIdColumnAsync(ct);
        var stateLedger = sp.GetRequiredService<Prose.Core.Services.WorldStateLedger>();
            ...
            if (orphans.Count > 0)
            {
                ...
                foreach (var grp in orphans.GroupBy(o => o.UniverseId))
                {
                    ...
                    if (drafts == null)
                    {
                        ...
                        drafts = new Prose.Core.Data.Entities.BookNode
                        {
        ...
        if (withCacophonySeed)
        {
            ...
            var linker = sp.GetRequiredService<Prose.Core.Services.AmmunitionLinkerService>();
        ...
        if (withState)
        {
            ...
            var ledger    = sp.GetRequiredService<Prose.Core.Services.WorldStateLedger>();
            var extractor = sp.GetRequiredService<Prose.Core.Services.BeatStateExtractor>();
            var chapters  = sp.GetRequiredService<Prose.Core.Interfaces.IChapterRepository>();
        ...
        if (withMojibake)
        {
            ...
            var fixer = sp.GetRequiredService<Prose.Core.Services.MojibakeRepairService>();
            var progress = new Progress<string>(Console.WriteLine);
