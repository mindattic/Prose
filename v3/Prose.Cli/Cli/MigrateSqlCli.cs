...
/// appsettings <c>ConnectionStrings:Prose</c> → LocalDB.
/// </summary>
public static class MigrateSqlCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider sp)
    {
        ...
        Console.WriteLine("=== Prose SQL migration ===");
        var failures = 0;
        ...
        if (charRelational)
        {
            ...
            if (!charNoBackfill)
            {
                ...
                {
                    var logRoot = Path.Combine(Path.GetTempPath(), "prose_migrate");
...
#pragma warning restore EF1002
        ...
        if (entityReviews)
        {
            ...
            var erDb = erScope.ServiceProvider.GetRequiredService<ProseDbContext>();
            Console.WriteLine();
        ...
        if (emotionalExamination)
        {
            ...
            var eeDb = eeScope.ServiceProvider.GetRequiredService<ProseDbContext>();
            Console.WriteLine();
