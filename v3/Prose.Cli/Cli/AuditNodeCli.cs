...
public static class AuditNodeCli
{
    public static async Task<int> RunAsync(IServiceProvider sp, string[] args)
    {
        ...
        if (slug == null) { Console.Error.WriteLine("Usage: ss --audit-book --slug <slug> [--deep] [--model <id>] [--out <path>]"); return 2; }
        ...
        W($"_Generated by `ss --audit-book`{(deep ? $", deep tier, model={model ?? "default"}" : ", free tier")}._");
        W();
        ...
        if (totEq == 0)          gaps.Add("EmotionalScore never populated (run --audit-book --deep, or ss --examine-emotion).");
