...
/// <summary>
/// prose --set-book-outline --slug &lt;slug&gt; --file &lt;path&gt;
///
/// CLI mirror of the MCP tool SetBookOutline (Tools.Nodes.cs) — hand-write the node bible text
...
/// </summary>
public static class SetBookOutlineCli
{
    public static async Task<int> RunAsync(string[] args, IServiceProvider services)
    {
        ...
        if (string.IsNullOrWhiteSpace(slug) || string.IsNullOrWhiteSpace(filePath))
        {
            ...
            Console.Error.WriteLine("Usage: prose --set-book-outline --slug <slug> --file <path-to-bible.md>");
            return 2;
        }
        ...
        var nodeBible = services.GetRequiredService<NodeOutlineService>();
        ...
        // in lockstep with Nodes.NodeOutline. Writing only the latter left NodeOutlineSections stale,
        ...
        // did — only the LLM-generation path (GenerateBookOutline) called ParseBeatSpine. Fixed by
        // routing through the same planned-beat creation NodeOutlineService.GenerateAndSaveAsync
        ...
        Console.WriteLine($"[set-book-bible] Saved {bibleText.Length} chars to Nodes.NodeOutline for '{node.Title}' ({node.Slug}).");
