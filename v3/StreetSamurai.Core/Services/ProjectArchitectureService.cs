...
/// <summary>
/// Scans the source tree (v3/StreetSamurai.*, root .js, tools/, engine/scripts/)
...
public class ProjectArchitectureService
{
    ...
    static readonly string[] ServiceProjects = ["StreetSamurai.Core", "StreetSamurai.Shared", "StreetSamurai.Blazor"];
    ...
    {
        var file = Path.Combine(v3, "StreetSamurai.Core", "Extensions", "ServiceCollectionExtensions.cs");
        var text = SafeRead(file);
        if (text == null) return [];
    ...
    private List<RazorPageEntry> ScanRazorPages(string v3)
    {
        ...
        foreach (var proj in new[] { "StreetSamurai.Shared", "StreetSamurai.Blazor" })
        {
    ...
    {
        var dir = Path.Combine(v3, "StreetSamurai.Blazor", "Cli");
    ...
    {
        var dir = Path.Combine(v3, "StreetSamurai.Mcp");
        if (!Directory.Exists(dir)) return [];
