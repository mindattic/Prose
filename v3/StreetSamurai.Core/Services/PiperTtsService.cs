...
/// in <see cref="StrandWorkbenchService.PublishAudiobookAsync"/> works unchanged.
...
///    <c>tools\piper</c>);
/// 3. <c>%LOCALAPPDATA%\StreetSamurai\piper</c>.</para>
/// </summary>
public sealed class PiperTtsService : ILocalTtsEngine
{
    ...
    public PiperTtsService(ILogger? log = null)
    {
        ...
        var exe = Environment.GetEnvironmentVariable("SS_PIPER_EXE");
        var model = Environment.GetEnvironmentVariable("SS_PIPER_MODEL");
    ...
    public async Task<byte[]> SynthesizeToPcmAsync(string text, string ffmpegPath, CancellationToken ct = default)
    {
        if (!IsAvailable) throw new InvalidOperationException(
            ...
            "or set SS_PIPER_EXE / SS_PIPER_MODEL.");
