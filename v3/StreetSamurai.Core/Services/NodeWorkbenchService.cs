...
public class NodeWorkbenchService
{
    ...
    private static async Task WalkAsync(StreetSamuraiDbContext db, Guid nodeId, List<OrderedBeat> acc, HashSet<Guid> visited, bool includeDisabled, CancellationToken ct)
    {
    ...
    private async Task<(Guid Id, string Slug)> CloneNodeSubtreeAsync(
        StreetSamuraiDbContext db, Guid srcNodeId, string? titleOverride,
