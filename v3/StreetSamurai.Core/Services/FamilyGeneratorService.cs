...
public class FamilyGeneratorService
{
    ...
    /// </summary>
    private async Task<SpouseDonor?> PickSpouseDonorAsync(StreetSamuraiDbContext db,
    ...
    private async Task<NamePool> BuildNamePoolAsync(StreetSamuraiDbContext db, Guid excludeId,
    ...
    private async Task WriteStubCharacterAsync(ProposedCharacter p, CancellationToken ct)
    {
        ...
        var plainSlug = StreetSamurai.Core.Services.WorldGraphService.Slugify(fullName);
        var slug = plainSlug;
