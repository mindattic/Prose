...
public class EpisodeSeedService
{
    ...
    private async Task<Guid?> PickRandomEntityIdAsync(
        StreetSamuraiDbContext db, string entityType, string? excludeName, CancellationToken ct)
    {
    ...
    private static async Task<string> GetNameAsync(StreetSamuraiDbContext db, Guid id, CancellationToken ct)
    {
