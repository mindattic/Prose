...
public static class EntertainmentMapper
{
    ...
    /// </summary>
    public static List<EntertainmentData> LoadAllLite(StreetSamuraiDbContext db)
    {
        var rows = db.EntertainmentItems.AsNoTracking()
            .Join(db.Entities.AsNoTracking().Where(e => e.IsActive && e.EntityType == "entertainment"),
                ei => ei.Id, e => e.Id,
    ...
    /// </summary>
    public static List<EntertainmentData> LoadAll(StreetSamuraiDbContext db, bool includeArchived = false)
    {
        var ids = (includeArchived
            ? db.Entities.AsNoTracking().Where(e => e.EntityType == "entertainment")
            : db.Entities.AsNoTracking().Where(e => e.EntityType == "entertainment" && e.IsActive))
            .Select(e => e.Id)
            .ToHashSet();
    ...
    public static EntertainmentData? LoadOne(StreetSamuraiDbContext db, Guid id)
    {
    ...
    /// </summary>
    public static async Task PersistAsync(StreetSamuraiDbContext db, Guid id, EntertainmentData src, CancellationToken ct = default)
    {
    ...
    public static void FillBridges(StreetSamuraiDbContext db, Guid id, EntertainmentData src)
    {
    ...
    /// Run once via <c>ss --rebuild-entertainment-relational</c>.
    /// </summary>
