...
1. **Foundation** (this session): packages, `StreetSamuraiDbContext`, schema migration, `JsonImportService` skeleton, `Character` end-to-end with dual-write toggle.
...
- [ ] Create `Data/` namespace with `StreetSamuraiDbContext` + entity classes (universal layer first, then `Character`).
