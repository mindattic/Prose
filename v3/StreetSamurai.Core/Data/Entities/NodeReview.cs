...
/// <summary>
/// One persona-based reader review of a <see cref="Node"/>. A Legion persona
/// (from the 1000-persona library) reads the whole node and, IN CHARACTER,
/// writes an honest review with a 1-100 score plus concrete improvement notes —
/// like a reader on a book site. Append-only; a node accrues many reviews
/// across runs.
///
/// Nodes/Beats are NOT system-versioned (vector-index incompatible), so there
...
/// VERSION of the node it was written against and flagged stale after edits.
/// </summary>
public class NodeReview
{
    ...
    /// <summary>Factual contradictions spotted in this node (one per line): internal
    ...
    /// identifies which version of the node this review is about.</summary>
    ...
    public List<NodeReviewBeatScore> BeatScores { get; set; } = new();
