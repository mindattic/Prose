...
SET @RowsThisBlock = @@ROWCOUNT;
PRINT CONCAT('Block A rows updated: ', @RowsThisBlock);
IF @RowsThisBlock <> 56 BEGIN ROLLBACK TRANSACTION; THROW 51000, 'Block A row count mismatch, rolled back.', 1; END
SET @TotalRows = @TotalRows + @RowsThisBlock;
...
-- 9000.0). Orphan 4392 relocated to the front of the block (see plan §1).
-- =====================================================================
UPDATE nb
SET nb.SortKey = v.NewSortKey
FROM NodeBeats nb
...
WHERE nb.NodeId = @NodeId AND nb.IsEnabled = 1;
SET @RowsThisBlock = @@ROWCOUNT;
PRINT CONCAT('Block B rows updated: ', @RowsThisBlock);
IF @RowsThisBlock <> 40 BEGIN ROLLBACK TRANSACTION; THROW 51001, 'Block B row count mismatch, rolled back.', 1; END
SET @TotalRows = @TotalRows + @RowsThisBlock;
...
WHERE nb.NodeId = @NodeId AND nb.IsEnabled = 1;
SET @RowsThisBlock = @@ROWCOUNT;
PRINT CONCAT('Block C rows updated: ', @RowsThisBlock);
IF @RowsThisBlock <> 38 BEGIN ROLLBACK TRANSACTION; THROW 51002, 'Block C row count mismatch, rolled back.', 1; END
SET @TotalRows = @TotalRows + @RowsThisBlock;
...
SET @RowsThisBlock = @@ROWCOUNT;
PRINT CONCAT('Block D rows updated: ', @RowsThisBlock);
IF @RowsThisBlock <> 1 BEGIN ROLLBACK TRANSACTION; THROW 51003, 'Block D row count mismatch, rolled back.', 1; END
SET @TotalRows = @TotalRows + @RowsThisBlock;

PRINT CONCAT('Total rows updated across all blocks: ', @TotalRows);
IF @TotalRows <> 135 BEGIN ROLLBACK TRANSACTION; THROW 51004, 'Total row count mismatch, rolled back.', 1; END
