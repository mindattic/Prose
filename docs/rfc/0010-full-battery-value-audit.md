...
| 3 — diet rule for the rest | **done.** The author approved all three groups and the fourteen standalone never-applied instruments are gone: group A `1cb5a9489` (sacred-flaw, dramatic-question, five-act-map, theme-coherence), group B `566d030fe` (swain-audit, chekhov-audit, diagnose-book, check-fidelity, check-canon), group C `1bb0e362f` (altitude-audit, pov-audit, hook-audit + the automatic chapter-close hook, craft-checklist, examine-emotion + its daily sweep, storyscope-audit, commandment-audit, plant-audit) — with the CLAUDE.md / LOGIC.md / READER-QA.md / skill text that named them rewritten in the same commits. **Kept because something live depends on it:** `SemanticFidelityService` (the per-beat intent-drift check inside the writer — the writer RFC's business), `StructuralDiagnosticService` (the SS-A44-quarantined panel's pre-flight), `CanonContradictionService` (chapter close), `PlantPayoffService` whole (writer plant block, sweep dimension 4, register/link tools), `BookAuditService.BuildCommandmentContext` (writer), the `ConsensusCliches` table (the blueprint reads it). The two deterministic native craft rules validated on 2026-09-06 (`InteriorityDensityRule`, `RetiredTicRule`) were lifted out of the deleted checklist into `CraftNativeRules.cs` and now run inside `--lint-prose` (`8c82b8176`) — same LINT prefix, no LLM. Day total across the battery and the three groups: **64 files, −10,190 lines**; the publish gate re-read 5/5 on the final Hub. |
