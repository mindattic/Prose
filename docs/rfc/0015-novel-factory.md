...
- **An incidental ruling now also suppresses the surface in that book's scan** (`EntityMentionScanner`, 2c111fb56). A common word that happens to be a vocabulary term or a handle's fragment ("Cut", "Heat", "Rig", "the south side") is fixed with a BCODA `incidental` ruling and `--factory capture --retag-name <surface> --from <id>`. The removal reports exit 3 if the scan would put the tag back. A tag on the wrong referent moves with `--to <id>`. Pins keep a moved tag in place across saves.
- **A first-name collision is fixed in the prose, not the record.** Following the author's precedent (the fleshed-out character keeps the name), BCODA's minor people were renamed, each new name checked unique across the corpus with `--find-entity`: Freya→Odile, Marcus Tsui→Horace Tsui, Dex→Jory, Rook→Lintel, Renata→Yaaba Osei-Bonsu, Voss→Dents, Emre→Kojo, Echo→Sonar and Farida Okonkwo→Zuleika Kowalczuk. One-scene names get an `incidental` ruling; recurring ones get a book-scoped entity (`originNodeSlug`), which wins the surface in its own book.
- **A record fix is checked against the whole book, not the beat that raised it.** Examining records against their mention packets found facts the book contradicts. Examples: Mrs. Chen's "delivery" covenant and dead son; the Atlas array's 2194 install; Operator Five dead (he walked); "Carrion Enterprises"; Mercer's "sixty-one ghost signatures". The fixes are field writes read back through `set_character_fields` / `set_entity_fields`. `location` still lives in `EntityStateEvents`, so it is corrected with `--fix-location-aspect`.
- **Relationships compare as a set in the read-back** (fe73fcc6f). The store loads them sorted by name, so a correct write had reported `NotLanded`. `--factory capture --retag` now exits on the scan its own repair produced.
- **A prose line that explains away an apparent error is suspect.** "Antiquity & Stationary" was defended on the page as deliberate. The author ruled it a typo: the spelling is now Stationery everywhere, the explanation is gone, and a `law` pattern (01a0ce92) keeps it from coming back.
- **What F4 still cannot see:** a name of three letters or fewer (Cho, Lin, Mei, KT), or one used in a single beat, is never flagged. A surname shared with another book's character (Cho) therefore passes unmarked. Nor can it see a mention tagged to the wrong record: in the BCODA reader's read, a splice re-derived "Stash" onto another book's Dr. Jude "Stash" Adeyemi instead of BCODA's Doc Stash, and F4 passed because the mention was tagged, only to the wrong record. F1 verification caught it (#20351). The author later ruled the two Stashes one person; the limit stands. All three are recorded as limits, not patched with a new instrument (author, 2026-09-23, order 01a0d073).
