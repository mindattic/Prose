...
- **What F4 still cannot see:** a name of three letters or fewer (Cho, Lin, Mei, KT), or one used in a single beat, is never flagged. A surname shared with another book's character (Cho) therefore passes unmarked. This is recorded as a limit, not patched with a new instrument.
