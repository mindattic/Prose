...
A ghost from the last job, a locked room inside someone's head, and the cost of infrastructure nobody maintained.
...
Seun Adalemo survived the lake. PEREGRINE did not dissolve — it reconstituted under new paper, funded not by Axiom BioNanics (who is now hemorrhaging litigation and stock value) but by a Nano Triumvirate competitor moving to acquire Axiom's destabilized Class V research division before the dust settles. Adalemo's new contract is not recovery. It is acquisition — specifically, the one person who understood the Class V pathway better than anyone now that the formula is public: Ohara.
...
What is in the partition when it opens is not what Ohara theorized. It is older. It is from before she locked it. She did not lock it to protect it from Axiom.

She locked it to protect herself from it.

### Act Four — The Accounting
The decision gets made, with consequences that cannot be filed away. Adalemo appears — not only as antagonist. The final scene is Rook at a different diner, a different window, a different number to count. Neon off the lake at night, doing what neon does: reflect and mean nothing and look like everything.
...
| 3 | **Ohara's Briefing** | Full scope — the partition, the hardware degradation timeline, the working theory about what is inside; the window is weeks, possibly less |
| 4 | **Vox's Read** | Four-channel network sweep; PEREGRINE's new paper and new funder identified; the competitor is moving fast; the window is shorter than Ohara estimated |
...
| 11 | **The Procedure** | Stave works the partition from outside; Ohara holds still while the lock degrades in real time; the open comes wrong — older data, locked by Ohara's own hand, for reasons she had forgotten she had |
| 12 | **The Contents** | What was inside; what it means; the decision that has to be made before PEREGRINE gets through the door |
...
| Strand | Code | DB ID | Writer | Endpoint |
|---|---|---|---|---|
...
FROM Beats b
INNER JOIN StrandBeats sb ON sb.BeatId = b.Id
WHERE sb.StrandId IN ('<nrstc-id>', '<nrstq-id>')
...
After both strands are reviewed:
...
```
ss --review --slug neon-rust-019f06da
ss --review --slug neon-rust-qwen-019f06eb
```
...
**Rook's arc:** She came back because the note made it easy. That is the problem. Her maintenance protocol — the rule that kept her clean — degraded once because convenience allowed it, and now she is here for a second job with the same crew against her own stated design. The strand does not fix this. It makes her look at it directly. The final diner scene is her at a different window with a different number, and what is different is not just the amount.
