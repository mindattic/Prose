...
- **`Universe`** — the top-level tenant: a lookup row (`Id, Slug, Name, Description, Theme,
  IsActive`) naming one fictional world (e.g. `glmz`, `scry`). **Every canon and story
...
**Scry narrative laws (Universe: scry — validate any Scry/Caul rewrite against these). World facts: `docs/universes/CAUL.md`.**
...
19. **Death is permanent in the Fantasy universe.** {#SS-LAW-19} In Universe #2 (The Caul), consciousness does not transfer between bodies. There is no shell-cycle, no spark-state, no return from Bheur. A Myrmidon who dies is gone. Transmutation (guided Catalyst infusion) changes the body of the living person — it does not relocate consciousness; a Transmuted soldier who dies is gone like any other. The theological debate about what follows death (Bheur, the Holding) is unresolvable because no one returns to report. Prose must not imply otherwise: no character survives death in a new body, returns from Bheur as a living agent, or carries memories across a death boundary. Full world facts: `docs/universes/CAUL.md`.
...
26. **Operator class vocabulary is fixed.** {#SS-LAW-26} *(2026-07-11; formerly Exo/NSB/RFO/Rider — all retired.)* Three gray-zone operator classes exist in GLMZ and must be referenced by these names only: **Channeler** (session-injection into networked frames; direct AI negotiation), **Ghost** (eigenstate conscience transfer — inhabiting any machine with coherent circuitry; formal term: ECT; the body at rest during a ghost = **Husk**), **Splicer** (hardware/software attack surface, credential work, physical build). "Rider", "Exo", "RFO" (Remote Frame Operator), and "NSB" (Neuretic Substrate Bridging) are **retired** — do not use them in prose or dialogue. The street term for casual use of the Ghost ability is *ghosting* (lowercase, as a verb: "she ghosted the stoplight green"). *(GLMZ.)*
28. **Every manufactured object in GLMZ carries a brand.** {#SS-LAW-28} *(2026-07-11.)* Brand names are mandatory in GLMZ prose. No character owns a "combat shotgun" — they own a Carrion Defense Works CDW-RS5. No one rides a "motorcycle" — they ride an Aries Viper Predator MDK-2. The commercial layer is inescapable inside the archologies and filters down through the Veil as knockoffs, secondhand product, and gray zone reclaim. Four brand tiers define the texture: **Archology-tier** (licensed CorpoNation product, quality-controlled, aspirational from below), **Licensed street** (official product sold below the Veil; older model or reduced spec; omnipresent), **Gray zone reclaim** (secondhand, modified, or counterfeit; may outperform corpo in specific applications; no warranty), **Street custom** (locally fabricated from scavenged components; no brand; often the best thing in the room). Character loadouts must name manufacturers and models. Neuretic-augmented characters automatically identify brand names via their overlay — they do not choose to notice brands; they cannot stop noticing them. Prose for augmented POV characters should reflect this: looking at gear means knowing what it is. Products also project AR holographic effects in shared space via embedded emitters on a neuretic-readable AR channel — a parked bike trails a chromatic ghost of itself, a jacket throws a brand ident into the air as its wearer passes. These effects are technically dismissible one at a time, but at archology street density there are hundreds simultaneously; nobody suppresses them. You learn to see through holograms like rain. The gray zone strips both layers: no neural ad injection, no AR emitters — augmented characters arriving from inside the Veil notice the absence before they notice anything else. The gray zone is the only geography left where the brand layer is optional — and operators who can afford archology-tier gear often choose not to wear it, because a Casque & Veil jacket reads as corpo affiliation and immediately flags in every NCID overlay within line of sight. *(GLMZ; applies to all GLMZ prose.)*
29. **"Hyper Reality" is the canonical GLMZ term for all neuretically-constructed perception.** {#SS-LAW-29} *(2026-07-11.)* Anything seen, heard, or felt through neuretic augmentation that has no physical counterpart is **Hyper Reality** — not a metaphor, not slang, not a value judgment. The Pulse train interior is the canonical example: passengers ride sealed pitch-black acceleration spheres; their neuretics surface a shared vintage train car (seats, aisles, other passengers visible across the aisle). The train is the experience; the pod is engineering. In archology zones, physical infrastructure is built as Hyper Reality substrate — surfaces optimized for AR rendering and machine vision, not inhabitation. Strip the overlay in an archology corridor and you find engineered blankness. The gray zone has no substrate optimization and no AR emitter infrastructure; augmented characters entering from inside the Veil notice the Hyper Reality going quiet before anything else. Unaugmented characters live permanently in the substrate — the blank, purposeless physical world the archologies were not designed to be experienced without an overlay. Use "Hyper Reality" in prose the way characters in 2226 use it: as a noun for the totality of the overlay experience, used naturally in dialogue and narration without definition. **Every scene set in a city environment (archology corridors, GLMZ streets, any built space inside the Veil) must include at least one ambient Hyper Reality texture detail** — it need not be a named product. A seltzer can sprays animated pixelated bubbles when opened. A wall-panel cycles through its ad-rotation. A transit counter dissolves and respawns. The civic infrastructure runs on the Hyper Reality layer too: holographic stop signs hang at intersections (legally binding, neuretic-readable), semi-transparent blue barriers designate pedestrian queues, AR traffic cones mark construction zones and incident perimeters, crowd-flow arrows pulse on the ground in transit corridors. All managed by Waxwing Neuromedia under the Corridor Authority municipal contract. For the augmented majority, these are as authoritative as physical infrastructure. For the unaugmented, none of it exists. The world is illusions on top of illusions. Gray zone scenes invert this: absence of Hyper Reality is the texture. *(GLMZ.)*
27. **GLMZ enforcement split: Arcturus / ArcSec (physical) and Halcyon / NCID / RaNCID (neuretics-crime).** {#SS-LAW-27} *(2026-07-11.)* The Meridian Compact charters two distinct enforcement monopolies in the GLMZ corridor. **Arcturus Defense Solutions**, known operationally as **ArcSec** (Arcturus Military Command), is the *physical* security monopoly: sovereign perimeter defense, PMC contracting, corporate military, autonomous weapons. **Halcyon Civil Security**, through its investigative field arm the **Neuretic Crime Investigation Division (NCID)** — street name **RaNCID** — is the *neuretics-crime* enforcement monopoly: corridor-wide investigation of crimes where neuretics are the weapon or method (illegal ghosting / ECT, unauthorized neural surveillance, black ice deployment, neuretics assault). Halcyon's power is administrative and infrastructural — they control augmentation certification and maintenance across the corridor, giving NCID forensic access no other body can match. The two monopolies do not compete on each other's terrain. *(GLMZ; applies to all GLMZ stories featuring enforcement, corporate security, or CorpoNation investigators.)*
...
- **Ghost** *(GLMZ)* — operator class; performs eigenstate conscience transfer (ECT): conscious perception transferred into any machine with coherent circuitry. Retired synonyms: Exo, Rider, RFO. See [SS-LAW-26](#SS-§5).
...
- **Channeler** *(GLMZ)* — operator class; session-injection into networked frames; also direct AI negotiation (undocumented edge). See [SS-LAW-26](#SS-§5).
- **Splicer** *(GLMZ)* — operator class; hardware/software attack surface, credential cracking, physical build. See [SS-LAW-26](#SS-§5).
- **Husk** *(GLMZ)* — a Ghost operator's body at rest during an eigenstate conscience transfer.
- **ArcSec** *(GLMZ)* — street and industry shorthand for Arcturus Defense Solutions (formally: Arcturus Military Command); the *physical* security monopoly in the GLMZ corridor. See [SS-LAW-27](#SS-§5).
- **NCID** *(GLMZ)* — Neuretic Crime Investigation Division; Halcyon Civil Security's investigative field arm for neuretics-crime: illegal ghosting / ECT, unauthorized neural surveillance, black ice deployment, neuretics assault. Organized into geographic field divisions (e.g. GLMZ Field Division). Street name: **RaNCID**. See [SS-LAW-27](#SS-§5).
