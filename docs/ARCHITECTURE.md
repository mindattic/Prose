# Prose â€” System Architecture

> **Replaces the previous version of this file (superseded 2026-06-07, deleted content centered on
> `StoryDirectorService` and a Blazor `Write.razor` UI â€” both deleted 2026-08-13, commit
...
> universal prose craft rules, see `docs/ENGINE.md` (Tier 0) and `docs/CRAFT.md` (Tier 1) â€” this
...
| Project | Kind | Role |
|---|---|---|
| **`Prose.Core`** | Library | Everything: `ProseDbContext`, every service (enrichment chain, WriteGate, DCM, audits, generation), EF migrations. No entry point of its own â€” referenced by all three below. |
| **`Prose.Hub`** | ASP.NET Core app (`WebHost.UseUrls("http://127.0.0.1:5900")`) | **The one resident, long-running process.** Owns the DI container every write and most reads actually execute inside. Hosts `CliDispatch`/`ToolDispatch` (reflection-based command execution), the WriteGate hook, the resident Trinity (Â§3), and a small set of HTTP endpoints (`/api/health`, `/api/dcm/status`, `/api/entities/active`, `/api/edges`). Runs with a visible console window that echoes every dispatched command in/out (`HubConsoleEcho`, added 2026-08-21/22). |
| **`Prose.Cli`** | Console exe | The `prose` / `Prose.Cli.exe` command line. **Almost entirely a forwarder**: confirmed 218 of 220 handler call sites in `Program.cs` call `HubCliClient.ForwardAsync(...)`, which POSTs to the running Hub and streams its console output back â€” the actual `RunAsync` handler classes under `Prose.Cli/Cli/*.cs` (257 files) execute *inside the Hub process*, not inside the `Prose.Cli.exe` process that was invoked. Two commands run in-process instead (`WorkerModeCli`, `EstimateCostCli`) â€” neither touches shared state. |
| **`Prose.Mcp`** | Library (loaded into Hub) | MCP tool definitions (40 `Tools.*.cs` files, 309 `[McpServerTool]`-attributed methods, `docs/MCP_TOOLS.md` auto-generated). `Prose.Hub` loads this DLL directly â€” `ToolDispatch` reflects into it the same way `CliDispatch` reflects into `Prose.Cli`. |
| `Prose.Hub.Contracts` | Library | Shared DTOs (`ObservabilityDtos.cs`) between Hub and ObserverUi â€” kept separate so ObserverUi doesn't need to reference all of Core. |
| `Prose.ObserverUi` | Blazor (Razor) app | A live observability dashboard over Hub's own state (`HubApiClient`) â€” part of the 2026-08-20 "Observability plan," distinct from the deleted `Write.razor` authoring UI. Watches, does not author. |
...
| `Prose.UnitTests` | Test project | 2176 tests as of 2026-08-23 (14 pre-existing, unrelated failures â€” `LoggingServiceTests` Serilog-format parsing + 2 `DI_Resolves*Tools` tests missing a `HubInvoker` test registration). |

**The system that matters for "does a write get validated, does a read see fresh state" is
Cli â†’ Hub (â†’ Mcp) â†’ Core.** Everything else is peripheral tooling.

## 2. The dispatch model â€” reflection, not routing
...
Both write a `CommandLedgerEntry` for every invocation â€” this is the closest thing to a system
...
## 3. The resident Trinity â€” Hub's real, but narrow, memory

Hub genuinely holds long-lived in-memory state across separate CLI/MCP invocations â€” this is not
...
(Â§1), state set by one command really is still there for the next one:

| Component | What it holds | Registration |
|---|---|---|
| `DocContextStack` | The DCM working set â€” a `ConcurrentDictionary<Guid /*NodeId*/, ContextState>` with its own action counter and LRU eviction (`EvictAfterActions`) | `AddSingleton`, `ServiceCollectionExtensions.cs` |
| `EntityContextStack` | Entity-level LRU (the "Lyra vs Vega" rule â€” see CLAUDE.md's DCM section) | `AddSingleton` |
...
`ProseWriterRouter` (the beat-generation path, Â§4) ever read or write it. The other ~217 CLI/MCP
commands â€” entity CRUD, audits, sweeps, exports â€” neither consult it nor invalidate it. A write
...
There is **no routing intelligence** anywhere in the system â€” Hub never decides which service or
command to invoke; every dispatch is a caller (human via CLI flag, or an LLM via MCP tool name)
naming the exact target.

## 4. The prose-generation path â€” `ProseWriterRouter`
...
coordinates ~25 enrichment services, each gated on its own precondition â€” the full, corrected
table lives in **CLAUDE.md's "Context enrichment chain" section** (corrected 2026-08-23; do not
duplicate that table here, it will drift â€” this file points to it as the source of truth for that
specific mechanism).
...
and skipped the entire enrichment chain; it was **deleted 2026-08-23** as confirmed dead code â€”
...
- **Async, post-save, fire-and-forget**: dispatches changed entities to `IWriteAuditService`
  (`DefaultWriteAuditService`) for slower judgment-based checks â€” currently near-duplicate-entity
  detection, filed as `Finding` rows.

Both lists are wired once at Hub startup by `WriteGateBootstrap`, which **must be eagerly
resolved** in `Program.cs` (a singleton nobody resolves never constructs â€” this is the same "a
mechanism exists but nothing activates it" failure class the WriteGate initiative itself was built
to close; see Â§6). `WriteGateScope` is the ambient static gateway `ProseDbContext` reads from,
...
sanctioned service method, or a documented accepted exception) â€” see project memory for the full
per-file list.
...
rewired onto it** â€” so the old, buggier, or unvalidated path keeps running in production while
...
actually repointed* â€” not just that the new method compiles and has a test.

## 7. Canon documents are ALL database-backed â€” never hand-edit `docs/*.md`
...
--all`: every one of these files carries a `<!-- GENERATED â€” do not hand-edit -->` banner and a
real, non-trivial section count. **This file previously (in CLAUDE.md's Codex table) told readers
to hand-edit 5 of these files directly** â€” stale instructions from before they were migrated into
...
equivalent â€” editing canon content requires an MCP client connected to `Prose.Mcp`. A Claude Code
CLI session with no MCP server attached cannot correct canon content at all, only read/regenerate
the existing `.md` mirrors. Concretely: `docs/ENGINE.md` Â§SS-ENGINE-2 currently states there is
"no soft-delete anymore anywhere in the beat/node/link tables" and that `ArchivedBooks` snapshots
are the *only* recovery mechanism â€” this was true in the narrow window after
...
| Universal prose craft â€” Tier 0 (loads above everything, every beat) | `docs/ENGINE.md` |
| Universal prose craft â€” Tier 1 | `docs/CRAFT.md` |
| Per-universe craft â€” Tier 2 | `docs/GLMZ.md` / `docs/SCRY.md` |
