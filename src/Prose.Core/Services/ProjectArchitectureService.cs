...
/// <summary>
/// Scans the source tree (src/Prose.*, root .js, tools/, engine/scripts/) to produce an
...
public class ProjectArchitectureService
{
    ...
    private ArchitectureSnapshot BuildSnapshot()
    {
        ...
        {
            snapshot.Services = ScanServices(srcRoot);
            ...
            snapshot.CliCommands = ScanCli(srcRoot);
            snapshot.McpTools = ScanMcp(srcRoot);
            snapshot.DiRegistrations = ScanDiRegistrations(srcRoot);
        }
    ...
    private List<ServiceEntry> ScanServices(string srcRoot)
    {
        var list = new List<ServiceEntry>();
        foreach (var proj in ServiceProjects)
        {
            var dir = Path.Combine(srcRoot, proj, "Services");
    ...
    private List<DiRegistration> ScanDiRegistrations(string srcRoot)
    {
        var file = Path.Combine(srcRoot, "Prose.Core", "Extensions", "ServiceCollectionExtensions.cs");
        var text = SafeRead(file);
        if (text == null) return [];
    ...
    private List<CliCommand> ScanCli(string srcRoot)
    {
        var dir = Path.Combine(srcRoot, "Prose.Cli", "Cli");
    ...
    private List<McpTool> ScanMcp(string srcRoot)
    {
        var dir = Path.Combine(srcRoot, "Prose.Mcp");
        if (!Directory.Exists(dir)) return [];
    ...
    private List<ScriptEntry> ScanScripts(string repoRoot)
    {
        ...
        var dirs = new[]
        {
            (Folder: "src",            Path: Path.Combine(repoRoot, "src")),
