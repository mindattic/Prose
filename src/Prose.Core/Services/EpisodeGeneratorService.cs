...
/// engine/bushido_coda_src/00_style_guide.md when present. This keeps the
...
public class EpisodeGeneratorService
{
    ...
    private async Task<string> BuildUserPromptAsync(ProseDbContext db, DrawnSeed seed, CancellationToken ct)
    {
        var sb = new StringBuilder();
        sb.AppendLine("Tonight's adventure seed:");
        sb.AppendLine();
    ...
    private static async Task<string> GenerateUniqueSlugAsync(
        ProseDbContext db, string title, Guid episodeId, CancellationToken ct)
    {
