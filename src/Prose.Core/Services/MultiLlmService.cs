...
/// owned by <see cref="LegionClient"/>. This class keeps the Prose-
...
public class MultiLlmService
{
    ...
    public MultiLlmService(LegionClient legion, SettingsService settings)
    {
        ...
        providers =
        [
            new("claude", "Claude (API)", "https://api.anthropic.com/v1/messages",                                        settings.Model,           "anthropic"),
    ...
    public async Task<string> CallProviderAsync(string providerId, string system, string user, CancellationToken ct = default)
    {
        ...
        {
            Serilog.Log.Warning("[Prose] Legion circuit breaker open for {Provider}: {Message}", providerId, ex.Message);
            throw;
        }
        catch (HttpRequestException ex)
        {
            Serilog.Log.Error(ex, "[Prose] Legion call failed for {Provider} (model={Model}, status={Status})",
