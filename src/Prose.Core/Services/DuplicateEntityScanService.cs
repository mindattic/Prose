...
public class DuplicateEntityScanService(IDbContextFactory<ProseDbContext> dbFactory, ILlmService llm)
{
    ...
    public async Task<EntityMergeResult> MergeAsync(Guid winnerId, Guid loserId, CancellationToken ct = default)
    {
        ...
        // Entity tags live INSIDE prose as <entity … guid="…"> attributes — text, not
        ...
        // hash-gated consumers see the change honestly. Beats are system-versioned —
        ...
        return new EntityMergeResult(winnerId, loserId, relinked, deletedForCollision, undoLog, beatsRetagged);
    }
