...
public static class VocabularyMapper
{
    ...
    public static List<VocabularyData> LoadAll(ProseDbContext db, bool includeArchived = false)
    {
    ...
    public static VocabularyData? LoadOne(ProseDbContext db, Guid id)
    {
    ...
    /// </summary>
    public static async Task PersistAsync(ProseDbContext db, Guid id, VocabularyData src, CancellationToken ct = default)
    {
    ...
    public static async Task<int> RebuildAllAsync(ProseDbContext db, CancellationToken ct = default)
    {
        ...
        if (failed > 0)
            throw new InvalidOperationException($"VocabularyMapper.RebuildAllAsync: {failed} row(s) failed ({written} written) — see the log.");
        return written;
    }
}
