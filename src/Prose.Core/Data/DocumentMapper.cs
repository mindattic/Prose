...
public static class DocumentMapper
{
    ...
    /// </summary>
    public static List<WorldbuildingDocument> LoadAll(ProseDbContext db, bool includeArchived = false)
    {
    ...
    public static WorldbuildingDocument? LoadOne(ProseDbContext db, Guid id)
    {
    ...
    /// </summary>
    public static async Task PersistAsync(ProseDbContext db, Guid id, WorldbuildingDocument src, CancellationToken ct = default)
    {
    ...
    public static void FillBridges(ProseDbContext db, Guid id, WorldbuildingDocument src)
    {
    ...
    /// Run once via <c>prose --rebuild-document-relational</c>.
    /// </summary>
    public static async Task<int> RebuildAllAsync(ProseDbContext db, CancellationToken ct = default)
    {
        ...
        if (failed > 0)
            throw new InvalidOperationException($"DocumentMapper.RebuildAllAsync: {failed} row(s) failed ({written} written) — see the log.");
        return written;
    }
}
