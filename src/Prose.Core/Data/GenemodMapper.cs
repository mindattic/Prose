...
public static class GenemodMapper
{
    ...
    /// </summary>
    public static List<GenemodData> LoadAll(ProseDbContext db, bool includeArchived = false)
    {
    ...
    public static GenemodData? LoadOne(ProseDbContext db, Guid id)
    {
    ...
    /// </summary>
    public static async Task PersistAsync(ProseDbContext db, Guid id, GenemodData src, CancellationToken ct = default)
    {
    ...
    public static void FillBridges(ProseDbContext db, Guid id, GenemodData src)
    {
    ...
    public static async Task<int> RebuildAllAsync(ProseDbContext db, CancellationToken ct = default)
    {
        ...
        if (failed > 0)
            throw new InvalidOperationException($"GenemodMapper.RebuildAllAsync: {failed} row(s) failed ({written} written) — see the log.");
        return written;
    }
}
