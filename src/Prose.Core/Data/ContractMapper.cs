...
public static class ContractMapper
{
    ...
    public static List<ContractData> LoadAll(ProseDbContext db, bool includeArchived = false)
    {
    ...
    public static ContractData? LoadOne(ProseDbContext db, Guid id)
    {
    ...
    /// </summary>
    public static async Task PersistAsync(ProseDbContext db, Guid id, ContractData src, CancellationToken ct = default)
    {
    ...
    public static void FillBridges(ProseDbContext db, Guid id, ContractData src)
    {
    ...
    public static async Task<int> RebuildAllAsync(ProseDbContext db, CancellationToken ct = default)
    {
        ...
        if (failed > 0)
            throw new InvalidOperationException($"ContractMapper.RebuildAllAsync: {failed} row(s) failed ({written} written) — see the log.");
        return written;
    }
}
