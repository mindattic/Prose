...
public static class PlaceMapper
{
    ...
    public static List<DistrictData> LoadAllLite(ProseDbContext db)
    {
        var rows = db.Places.AsNoTracking()
            .Join(db.Entities.AsNoTracking().Where(e => e.EntityType == "place"),
                p => p.Id, e => e.Id,
    ...
    public static async Task<int> RebuildAllAsync(ProseDbContext db, CancellationToken ct = default)
    {
        ...
        if (failed > 0)
            throw new InvalidOperationException($"PlaceMapper.RebuildAllAsync: {failed} row(s) failed ({written} written) — see the log.");
        return written;
    }
