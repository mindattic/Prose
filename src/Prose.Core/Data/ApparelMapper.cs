...
public static class ApparelMapper
{
    ...
    /// </summary>
    public static List<ApparelData> LoadAllLite(ProseDbContext db)
    {
        var rows = db.Apparels.AsNoTracking()
            .Join(db.Entities.AsNoTracking().Where(e => e.EntityType == "apparel"),
                a => a.Id, e => e.Id,
    ...
    /// </summary>
    public static List<ApparelData> LoadAll(ProseDbContext db, bool includeArchived = false)
    {
    ...
    public static ApparelData? LoadOne(ProseDbContext db, Guid id)
    {
    ...
    /// </summary>
    public static async Task PersistAsync(ProseDbContext db, Guid id, ApparelData src, CancellationToken ct = default)
    {
    ...
    /// Run once via <c>prose --rebuild-apparel-relational</c>.
    /// </summary>
    public static async Task<int> RebuildAllAsync(ProseDbContext db, CancellationToken ct = default)
    {
        ...
        if (failed > 0)
            throw new InvalidOperationException($"ApparelMapper.RebuildAllAsync: {failed} row(s) failed ({written} written) — see the log.");
        return written;
    }
