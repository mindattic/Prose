...
using ContractEntity = Prose.Core.Data.Entities.Contract;
using DocumentEntity = Prose.Core.Data.Entities.Document;
using QuoteEntity    = Prose.Core.Data.Entities.Quote;
...
{
    public ProseDbContext(DbContextOptions<ProseDbContext> options) : base(options) { }
    ...
    /// </summary>
    public Guid ScopedUniverseId => Prose.Core.Services.UniverseScope.EffectiveId;
        ...
        foreach (var entry in ChangeTracker.Entries())
        {
            ...
            switch (entry.Entity)
            {
                ...
                case Setting st when st.UniverseId == Guid.Empty:
                    st.UniverseId = Prose.Core.Services.UniverseScope.SharedConfigKeys.Contains(st.Key)
    ...
    // Truth-First Architecture (Track A) — structured sections of the retired book outline.
    // PENDING DROP (author ruling 2026-09-22): no code reads/writes this; removed by the drop migration.
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        // Read receipts + notes (author ruling 2026-09-22): the only memory a read keeps.
        b.Entity<BeatReadReceipt>(e =>
        {
            e.ToTable("BeatReadReceipts");
            e.HasKey(x => x.BeatId);
            e.Property(x => x.TextHash).HasMaxLength(64).IsRequired();
