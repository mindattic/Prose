...
public static class PharmaceuticalMapper
{
    ...
    /// </summary>
    public static List<PharmaceuticalData> LoadAll(ProseDbContext db, bool includeArchived = false)
    {
    ...
    public static PharmaceuticalData? LoadOne(ProseDbContext db, Guid id)
    {
    ...
    /// </summary>
    public static async Task PersistAsync(ProseDbContext db, Guid id, PharmaceuticalData src, CancellationToken ct = default)
    {
    ...
    public static void FillBridges(ProseDbContext db, Guid id, PharmaceuticalData src)
    {
    ...
    public static async Task<int> RebuildAllAsync(ProseDbContext db, CancellationToken ct = default)
    {
        ...
        if (failed > 0)
            throw new InvalidOperationException($"PharmaceuticalMapper.RebuildAllAsync: {failed} row(s) failed ({written} written) — see the log.");
        return written;
    }
}
