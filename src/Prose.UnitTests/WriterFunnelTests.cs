...
public class WriterFunnelTests
{
    ...
    public void AllowList_EntriesStillExist()
    {
        ...
        var missing = BeatWriteAllowList.Keys.Where(k => !File.Exists(Path.Combine(root, "src", k.Replace('/', Path.DirectorySeparatorChar)))).ToList();
    ...
    private static string Rel(string path)
    {
        ...
        return Path.GetRelativePath(srcRoot, path).Replace('\\', '/');
    }
