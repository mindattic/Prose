...
{
    readonly IDbContextFactory<ProseDbContext> ownFactory;

    public SqliteWoundLedgerService(IDbContextFactory<ProseDbContext> dbFactory)
...
public class WoundLedgerServiceTests
{
    ...
    IDbContextFactory<ProseDbContext> factory = null!;
    ...
    sealed class PinnedSqliteFactory(DbContextOptions<ProseDbContext> options)
        : IDbContextFactory<ProseDbContext>
    {
        ...
        public Task<ProseDbContext> CreateDbContextAsync(CancellationToken ct = default)
            => Task.FromResult(new ProseDbContext(options));
    }
