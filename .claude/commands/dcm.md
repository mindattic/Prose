...
match means it's not up), **stop and tell the user the MCP path is unavailable — do not fall back
to a direct/raw/parameterized DB write of any kind.** Nothing reaches the database except through
Prose.Hub (HARD, absolute rule, 2026-08-22 — see project memory `feedback_all_writes_through_hub`).
An earlier version of this step told readers to fall back to a parameterized `sqlcmd`/`SqlClient`
write when MCP wasn't connected — that guidance caused a real incident and is retired. The user
decides whether to reconnect MCP, use the CLI instead, or grant a one-time documented exception.
