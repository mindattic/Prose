...
The deploy fires the project's existing GitHub Actions workflow (`azure-deploy.yml`) by committing the synced wwwroot and pushing master. Workflow then deploys to the `streetsamurai` Azure App Service (Production slot).
...
6. `git push origin master` — Azure CI/CD fires automatically.
