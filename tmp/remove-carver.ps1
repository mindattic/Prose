...
foreach($id in $ids){
  $raw=(& dotnet run --project src\Prose.Cli --no-build -- --beat show --id $id --universe glmz | Out-String)
  $lines=$raw -split '?
'
  $u=0
  ...
  Get-Content -LiteralPath $tmp -Raw | dotnet run --project src\Prose.Cli --no-build -- --beat update --id $id --text - --universe glmz | Out-Host
}
